...
async def cmd_rate(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """
    فرمت: /rate buy 3.82 sell 3.90 afn 71.5
    """
    if not is_admin(update.effective_user.id): return
    args = ctx.args
    try:
...
# ═══════════════════════════════════════════
# CONVERSATION STATE — ثبت حواله مرحله به مرحله
# ═══════════════════════════════════════════
user_state = {}
...
# ═══════════════════════════════════════════
# MESSAGE HANDLER — دریافت مراحل فرم
# ═══════════════════════════════════════════
...
async def callback_form(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Handle inline buttons during form filling"""
    query = update.callback_query
    await query.answer()
    uid = query.from_user.id
    data = query.data
